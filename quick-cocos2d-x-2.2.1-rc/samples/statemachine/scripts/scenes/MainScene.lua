...
function MainScene:ctor()
    ...
    self.fsm_:setupState({
        ...
        callbacks = {
            onbeforestart = function(event) self:log("[FSM] STARTING UP") end,    -- start事件前
            onstart       = function(event) self:log("[FSM] READY") end,            -- start事件后
            onbeforewarn  = function(event) self:log("[FSM] START   EVENT: warn!", true) end,  -- 事件前
            ...
            onwarn        = function(event) self:log("[FSM] FINISH  EVENT: warn!") end,         -- onafter event  事件后
            ...
            onleavegreen  = function(event) self:log("[FSM] LEAVE   STATE: green") end, -- 离开绿色状态时
            onleaveyellow = function(event) self:log("[FSM] LEAVE   STATE: yellow") end,    -- 离开黄色状态时
            ...
            ongreen       = function(event) self:log("[FSM] ENTER   STATE: green") end,      --  进入一个状态时
            onyellow      = function(event) self:log("[FSM] ENTER   STATE: yellow") end,
            onred         = function(event) self:log("[FSM] ENTER   STATE: red") end,
            onchangestate = function(event) self:log("[FSM] CHANGED STATE: " .. event.from .. " to " .. event.to) end,    -- 当状态发生改变的时候被激活
        },
    })
